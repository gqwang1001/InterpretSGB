...
simList = paste0("sim", c(32, 62, 43, 32, 7, 8)); scen =paste0("scenario",c(1:6)); cases="OrginalPaper"
...
for (isim in simList){
  ...
  for (ir in 1:NREP) {
    spltree = readRDS(paste0("/SFS/scratch/waguoqi2/InterPretSGBResults/", isim, "/", ir, ".rds"))
...
ggsave(paste0(savedir,"results/Accuracy_comparison_XgboostVsSplTree_", cases, ".pdf"), p1, width = 14)
...
p1 = ggplot(as.data.frame(sen.df.long),aes(x=Simulation, y=Sensitivity))+
  geom_boxplot(aes(fill=Method))
ggsave(paste0(savedir,"results/Sensitivity_comparison_XgboostVsSplTree_", cases, ".pdf"),p1, width = 14)
...
p1 = ggplot(as.data.frame(spe.df.long),aes(x=Simulation, y=Specificity))+
  geom_boxplot(aes(fill=Method))
ggsave(paste0(savedir,"results/Specificity_comparison_XgboostVsSplTree_", cases, ".pdf"),p1, width = 14)
...
p1 = ggplot(as.data.frame(rmse.df.long),aes(x=Simulation, y=RMSE))+
  geom_boxplot(aes(fill=Method))
ggsave(paste0(savedir,"results/RMSE_comparison_", cases, ".pdf"),p1, width = 14)
...
library(ggplot2)
dice.df.long = read.csv(paste0("results/dice_comparision_long_", cases, ".csv"))
...
p1 = ggplot(as.data.frame(dice.df.long),aes(x=Simulation, y=DICE))+
  geom_boxplot(aes(fill=Method))
ggsave(paste0(savedir,"results/dice_comparison_", cases, ".pdf"),p1, width = 14)
