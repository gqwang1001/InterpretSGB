rm(list=ls())
setwd("/SFS/scratch/zhapingy/sim42/code")
source("MySimData_sim42.R")
source("GBTE.R")
source("Myboost.R")
source("OneStep.R")
source("ROWSi.R")
...
MySim <- function(rep){
  ...
  data <- MySimData(N=5500,M=500,enrollment=12,follow=18,lambda0=0.1,p=50,rho=1/3,alpha0=0.4,eta=-log(.9)/12,mu1=1,mu2=-1,mu0=sqrt(6))
  ...
  res.GBT <- GBTE(y=dat.train$aval,  X=dat.train[,c(4:55)],  Tr=dat.train$trt01p,  newX=dat.test[,c(4:55)],  ntr=1000,response="S",Event=dat.train$evnt,th=20,otr=otr.test,dat.test=dat.test)
...
setwd("/SFS/scratch/zhapingy/sim42/res")
