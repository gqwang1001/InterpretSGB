rm(list=ls())
setwd("/SFS/scratch/zhapingy/sim62/code/")
source("MySimData_sim62.R")
source("Myboost2.R")
source("Myboost.R")
source("OneStep.R")
source("ROWSi.R")
...
MySim <- function(rep){
  ...
  data <- MySimData(N=5500,M=500,enrollment=12,follow=18,lambda0=0.1,p=50,rho=1/3,alpha0=sqrt(2),eta=-log(.9)/12,mu0=sqrt(6))
 ...
 #data <- SimData_Lu(N=5500,m=500,enrollment=12,follow=18,lambda0=0.1,p=100,rho=1/3,beta=c(1/sqrt(3),rep(0,10),rep(  1/(2*sqrt(3)),10 ),rep(0,80)),gamma=c(0.4,rep(c(0.8,-0.8),1),rep(0,98)),alpha12=0.8,alpha0=sqrt(2),eta=-log(.9)/12)
  ...
  res.boost.rmst1 <-MyBoost(dat=dat.train,otr=otr.test,dat.test=dat.test)
...
setwd("/SFS/scratch/zhapingy/sim62/res")
